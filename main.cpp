...
void sort_a (vector<int> &mass, int number)
{
...
void sort_b(vector<int> &mass, int left, int right)
{
    int k = left;
    int m = right;
    ...
    if (k < right) sort_b(mass, k, right);
    if (left < m) sort_b(mass, left, m);
}
